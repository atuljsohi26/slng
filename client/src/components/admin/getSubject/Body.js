...
const Body = () => {
  ...
  const getAllSubject = async () => {
    setLoading(true);
    try {
      const fetchSubject = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/admin/getAllSubject`,
        {
          method: "GET",
          headers: {
      ...
      const fetchSubjectResult = await fetchSubject.json();
      if (fetchSubjectResult.status) {
        setLoading(false);
        setSubject(fetchSubjectResult.response);
        toast.success(fetchSubjectResult.message);
      } else {
        setLoading(false);
        toast.error(fetchSubjectResult.message);
      }
    } catch (catchError) {
      setLoading(false);
